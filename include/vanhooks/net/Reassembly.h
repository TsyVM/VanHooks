...
// ===== Consolidated header: Reassembly.h =====
// Merges: IPReassembly, TcpReassembly
 ...
 * both IPv4 and IPv6 reassembly which means the same vanhooks::net::IPReassembly instance can reassemble both IPv4 and IPv6
 * fragments. You can read more about IP fragmentation here: https://en.wikipedia.org/wiki/IP_fragmentation.<BR> The API
 * is rather simple and contains one main method: vanhooks::net::IPReassembly#processPacket() which gets a fragment packet as a
 ...
 * being reassembled. The default limit is #VH_IP_REASSEMBLY_DEFAULT_MAX_PACKETS_TO_STORE but the user can set any
 * value (determined in vanhooks::net::IPReassembly c'tor). Once capacity (the number of concurrent reassembled packets) exceeds
 ...
 * packets are removed from the map by registering to the vanhooks::net::IPReassembly#OnFragmentsClean callback in
 * vanhooks::net::IPReassembly c'tor
 */
 ...
 */
#define VH_IP_REASSEMBLY_DEFAULT_MAX_PACKETS_TO_STORE 500000
	...
	public:
		...
		public:
			...
			/**
			 * @return The IP protocol this key represents (vanhooks::net::IPv4 or vanhooks::net::IPv6)
			 */
		...
		public:
			...
			/**
			 * @return vanhooks::net::IPv4 protocol
			 */
		...
		public:
			...
			/**
			 * @return vanhooks::net::IPv6 protocol
			 */
			ProtocolType getProtocolType() const
			{
				return IPv6;
		 ...
		 * determined in its c'tor (default value is #VH_IP_REASSEMBLY_DEFAULT_MAX_PACKETS_TO_STORE). When traffic
		 ...
		 * #VH_IP_REASSEMBLY_DEFAULT_MAX_PACKETS_TO_STORE
		 */
		explicit IPReassembly(OnFragmentsClean onFragmentsCleanCallback = nullptr, void* callbackUserCookie = nullptr,
		                      size_t maxPacketsToStore = VH_IP_REASSEMBLY_DEFAULT_MAX_PACKETS_TO_STORE)
 ...
 * - Manage multiple TCP connections under one vanhooks::net::TcpReassembly instance
 ...
 * - The user creates an instance of the vanhooks::net::TcpReassembly class
 * - Then the user starts feeding it with TCP packets
 * - The vanhooks::net::TcpReassembly instance manages all TCP connections from the packets it's being fed. For each connection it
 ...
 * - Then the vanhooks::net::TcpReassembly logic tries to understand if the data in this packet is the expected data
 ...
 *   creation of the vanhooks::net::TcpReassembly instance. This callback contains the new data (of course), but also information
 *   about the connection (5-tuple, 4-byte hash key describing the connection, etc.) and also a pointer to a "user
 *   cookie", meaning a pointer to a structure provided by the user during the creation of the vanhooks::net::TcpReassembly
 *   instance.
 ...
 * - vanhooks::net::TcpReassembly supports 2 more callbacks - one is invoked when a new TCP connection is first seen and the other
 ...
 *   meaning a pointer to a structure provided by the user during the creation of the vanhooks::net::TcpReassembly instance. The
 *   end connection callback also provides the reason for closing it ("naturally" or manually).
 *
 * __Basic Usage and APIs:__
 * - vanhooks::net::TcpReassembly c'tor - Create an instance, provide the callbacks and the user cookie to the instance
 * - vanhooks::net::TcpReassembly#reassemblePacket() - Feed vanhooks::net::TcpReassembly instance with packets
 * - vanhooks::net::TcpReassembly#closeConnection() - Manually close a connection by a flow key
 * - vanhooks::net::TcpReassembly#closeAllConnections() - Manually close all currently opened connections
 * - vanhooks::net::TcpReassembly#OnTcpMessageReady callback - Invoked when new data arrives on a certain connection. Contains the
 *   new data as well as connection data (5-tuple, flow key).
 * - vanhooks::net::TcpReassembly#OnTcpConnectionStart callback - Invoked when a new connection is identified
 * - vanhooks::net::TcpReassembly#OnTcpConnectionEnd callback - Invoked when a connection ends (either by FIN/RST or manually by
 *   the user).
 *
 ...
 * - vanhooks::net::TcpReassembly#reassemblePacket() should detect the packets that arrive after the FIN packet has been received.
 * - The user can use the information about connections managed by vanhooks::net::TcpReassembly instance. Following methods are
 *   used for this purpose: vanhooks::net::TcpReassembly#getConnectionInformation and vanhooks::net::TcpReassembly#isConnectionOpen.
 * Cleaning of memory can be performed automatically (the default behavior) by vanhooks::net::TcpReassembly#reassemblePacket() or
 * manually by calling vanhooks::net::TcpReassembly#purgeClosedConnections in the user code. Automatic cleaning is performed once
 * per second.
 *
 * The struct vanhooks::net::TcpReassemblyConfiguration allows to setup the parameters of cleanup. Following parameters are
 * supported:
 * - vanhooks::net::TcpReassemblyConfiguration#doNotRemoveConnInfo - if this member is set to false the automatic cleanup mode is
 *   applied.
 * - vanhooks::net::TcpReassemblyConfiguration#closedConnectionDelay - the value of delay expressed in seconds. The minimum value
 *   is 1.
 * - vanhooks::net::TcpReassemblyConfiguration#maxNumToClean - to avoid performance overhead when the cleanup is being performed,
 *   this parameter is used. It defines the maximum number of items to be removed per one call of
 *   vanhooks::net::TcpReassembly#purgeClosedConnections.
 * - vanhooks::net::TcpReassemblyConfiguration#maxOutOfOrderFragments - the maximum number of unmatched fragments to keep per flow
